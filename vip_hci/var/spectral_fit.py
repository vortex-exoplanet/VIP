...
def goodness_of_fit(lbda_obs, spec_obs, err_obs, lbda_mod, spec_mod, 
    ...
    def find_nearest(array,value,output='index',constraint=None):
        ...
        if constraint == 'ceil' and np.ravel(array)[idx]-value < 0 and idx<len(np.ravel(array))-1:
            idx+=1
        elif constraint == 'floor' and value-np.ravel(array)[idx] < 0 and idx>0:
            idx-=1
    ...
    if not np.array_equal(lbda_obs, lbda_mod):
        ...
        ## just limit the model to a section around the observed spectrum
            ...
            for i in range(1,len(instru_fwhm)+1):
                ...
                for ll, lbda in enumerate(lbda_obs[np.where(instru_idx==i)]):
                    ...
                    i_l = find_nearest(lbda_mod,
                                       mid_lbda_l[np.where(instru_idx==i)][ll+1])
    ...
    #chi_sq = np.linalg.multi_dot((wi*delta,np.linalg.inv(cov),wi*delta)) # ori 
...
def goodness_of_fit_mp(lbda_obs, spec_obs, err_obs, lbda_mod, spec_mod, 
                       dlbda_obs, instru_corr, instru_fwhm, instru_idx):
    """ Function to estimate the goodness of fit indicator defined as 
    in Olofsson et al. 2016 (Eq. 8). In addition, if a spectral 
    correlation matrix is provided, it is used to take into account the 
    correlated noise between spectral channels (see Greco & Brandt 2016).

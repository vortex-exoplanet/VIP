...
from vip_hci.preproc import cube_fix_badpix_clump
from vip_hci.preproc import cube_fix_badpix_ifs
...
def test_badpix_iso():
    ...
    cube_c, bpix_map = cube_fix_badpix_isolated(
        cube, sigma_clip=5, frame_by_frame=True, full_output=True
    )
...
def test_badpix_iso2():
    ...
    cube = np.array([im1, im2])
    cube_c, bpix_map = cube_fix_badpix_isolated(
        cube, sigma_clip=5, frame_by_frame=False, full_output=True
    )
...
def test_badpix_clump():
    ...
    cube = np.array([im1, im2])
    cube_c, bpix_map = cube_fix_badpix_clump(cube, sig=5, fwhm=6, full_output=True)
    ...
    cube_c, bpix_map = cube_fix_badpix_clump(
        cube, sig=5, fwhm=6, protect_mask=5, mad=True, full_output=True
    )
    ...
    # Test 2d input
    cube_c, bpix_map = cube_fix_badpix_clump(cube[0], sig=5, fwhm=6, full_output=True)
    assert bpix_map[idx0, idx0] == 1
...
def test_badpix_ann():
    ...
    cube = np.array([im1, im2])
    cube_c, bpm, _ = cube_fix_badpix_annuli(
        cube, fwhm=[4, 4.5], sig=5.0, full_output=True
    )
    ...
    cube_c, bpm, _ = cube_fix_badpix_annuli(
        cube, fwhm=[4, 4.5], sig=5.0, protect_mask=7, half_res_y=True, full_output=True
    )
...
def test_badpix_ifs1():
    ...
    for i in range(n_ch):
        cube[i] = fluxes[i] * create_synth_psf(
            shape=(sz, sz), model="moff", fwhm=fwhms[i]
        )
    ...
    # identify bad pixels
    cube_c, bpm, _ = cube_fix_badpix_ifs(
        cube,
        lbdas=fwhms / 2.0,
        clumps=False,
        sigma_clip=5.0,
...
def test_badpix_ifs2():
    ...
    for i in range(n_ch):
        cube2[i] = fluxes[i] * create_synth_psf(
            shape=(sz, sz), model="moff", fwhm=fwhms[i]
        )
    ...
    # protect mask + clumps
    cube_c2, bpm2, _ = cube_fix_badpix_ifs(
        cube2,
        lbdas=fwhms / 2.0,
        clumps=True,
        sigma_clip=4.0,

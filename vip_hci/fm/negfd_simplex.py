...
from .negfd_fmerit import chisquare_fd, get_mu_and_sigma_fd
...
def firstguess_fd_from_coord(planet, center, cube, angs, psfn, fwhm, annulus_width,
    ...
    def _grid_search_f(r0, theta0, ch, cube, angs, psfn, fwhm, annulus_width,
        ...
        for j, f_guess in enumerate(f_range):
            ...
            chi2r.append(chisquare_fd(params, cube, angs, psfn, fwhm, annulus_width,
...
def firstguess_fd_simplex(p, cube, angs, psfn, ncomp, fwhm, annulus_width,
    ...
    **kwargs: optional
    ...
    if force_rPA:
        ...
        p_t = p
    solu = minimize(chisquare_fd, p_t, args=(cube, angs, psfn, fwhm, annulus_width,
...
def firstguess_fd(cube, angs, psfn, ncomp, planets_xy_coord, fwhm=4,
                  ...
                  verbose=True, save=False, full_output=False):
    ...
    -------
    r_0, f_0 : float
        Radial shift of the model disk image
    theta_0: float
        Azimuthal rotation shift (with respect to x axis) for the model disk imageThe polar coordinates and the flux(es) of the companion.
    chi2: float
        [full_output=True] Corresponding
    ...
    for index_planet in range(n_planet):
        ...
        if isinstance(mu_sigma, tuple):
            ...
            r0 = np.sqrt(xy[0]**2 + xy[1]**2)
            theta0 = np.mod(np.arctan2(xy[1], xy[0]) / np.pi*180, 360)
            mu_sigma = get_mu_and_sigma_fd(cube, angs, ncomp, annulus_width,
        ...
        res_init = firstguess_fd_from_coord(planets_xy_coord[index_planet],
        ...
        if simplex or force_rPA:
            ...
            res = firstguess_fd_simplex(res_init, cube, angs, psfn, ncomp, fwhm,

...
@parametrize("r", [param(55), param(60), param(65)])
def test_dust_distribution(r):
    ...
    test.set_radial_density(ain=5.0, aout=-5.0, a=60.0, e=0.0)
    costheta = 0.0
    z = 0.0
...
@parametrize("i", [param(0), param(30), param(60)])
def test_scattered_light_disk(i):
    ...
    fake_disk = ScatteredLightDisk(
        ...
        spf_dico={"name": "HG", "g": 0.0, "polar": False},
        flux_max=1,
    )
...
@parametrize("g", [param(0), param(0.4), param(0.9)])
def test_HG_phase_function_and_print(g):
    ...
    dstar = 85.0  # distance to the star in pc
    PA = -75.0  # position angle in degree
    itilt = 80.0  # inclination in degree
    gamma = 2.0
    ksi0 = 1.0
    r0 = 24.0 * pixel_scale * dstar
    ain = 10.0
    aout = -4.0
...
@parametrize("g1", [param(0.2), param(0.4), param(0.9)])
def test_double_HG_phase_function(g1):
    ...
    dstar = 80.0  # distance to the star in pc
    PA = -12.0  # position angle in degree
    itilt = 10.0  # inclination in degree
    gamma = 2.0
    ksi0 = 1.0
    r0 = 24.0 * pixel_scale * dstar
    ain = 9.0
    aout = -6.0
    spf_dico = {"name": "DoubleHG", "g": [g1, -0.6], "weight": 0.7, "polar": False}
...
@parametrize("spf_50deg", [param(20), param(60), param(10)])
def test_homemade_phase_function(spf_50deg):
    ...
    dstar = 77.0  # distance to the star in pc
    PA = 27.0  # position angle in degree
    itilt = 78.0  # inclination in degree
    gamma = 2.0
    ksi0 = 1.0
    r0 = 24.0 * pixel_scale * dstar
    ain = 3.0
    aout = -11.0
    spf_angles = np.array([0.0, 50.0, 90.0, 130.0, 180.0])

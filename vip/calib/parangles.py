...
def compute_paral_angles(header, latitude, ra_key, dec_key, lst_key, acqtime_key, 
                         date_key='DATE-OBS'):                            
...
def compute_derot_angles_CD(objname_tmp_A, digit_format=3,objname_tmp_B='',
                            inpath='~/', skew=False, writing=False, outpath='~/', 
                            list_obj=None, cd11_key='CD1_1', cd12_key='CD1_2', 
                            cd21_key='CD2_1', cd22_key='CD2_2', verbose=False):
    """
    ...
    if list_obj is None:
        ...
        for ii in list_obj:
            digits_ii = numberToString(ii,digit_format)
            _, header = open_fits(inpath+objname_tmp_A+digits_ii+objname_tmp_B+'.fits', 

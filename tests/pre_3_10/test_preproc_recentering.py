...
from vip_hci.preproc import cube_recenter_radon
    ...
    **kwargs
    ...
    if debug:
        try:
            ...
            p = pd.DataFrame(
                np.array([shiftx, -unshiftx, shifty, -unshifty]).T,
                columns=["x", "un-x", "y", "un-y"],
            )
            ...
            print(" " * 18, -shifty)
        print(
            "\033[33merrors:\033[0m",
            mean_squared_error(shiftx, -unshiftx),
...
def test_dft(debug=False):
    ...
    method_args = dict(
        center_fr1=(mean, mean), subi_size=19, negative=True,
        **method_args_additional
    )
    do_recenter(
        method,

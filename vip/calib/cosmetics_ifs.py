...
def approx_stellar_position(obj_tmp, fwhm,return_test=False):
    """FIND THE APPROX COORDS OF THE STAR IN EACH CHANNEL (even the ones dominated by noise).
    If the approximate coordinates of the star found with peak_coordinates are not within 3sigma of the median position, it is replaced by the position in the closest good neighbouring channels.
    ...
    obj_tmp : cube_like
        Input 3d cube
    fwhm     : float_like
        Input full width half maximum value of the PSF. This will be used as the standard deviation for Gaussian kernel of the Gaussian filtering.
    return_test: {False,True}, Bool, optional 
        Indicates whether the test result vector should be returned as well, along with the approx stellar coordinates.

    Returns:
    --------
    star_approx_coords: array_like
        Array of x and y approx coordinates of the star in each channel of the cube
    If return_test is True, it returns as well the test result vector: a vector of same z dimension as the cube, filled with True (if the star position found with peak_coordinates had to be replaced by the position in closest good neighbouring channels) or False (otherwise, i.e. if the position found was within 3sigma of the median position).
    """
    ...
    _, med_y, stddev_y = sigma_clipped_stats(star_tentative_coords[:,0], sigma=2.5) 
    _, med_x, stddev_x = sigma_clipped_stats(star_tentative_coords[:,1], sigma=2.5)
    ...
    for zz in range(n_z):
        if (star_tentative_coords[zz,0] < lim_inf_y) or (star_tentative_coords[zz,0] > lim_sup_y) or (star_tentative_coords[zz,1] < lim_inf_x) or (star_tentative_coords[zz,1] > lim_sup_x):
            test_result[zz] = 1
    ...
    if return_test:
        return star_approx_coords, test_result.astype(bool)
    else:
...
def bad_pixel_removal(obj_tmp, center, fwhm, sig=5., protect_psf=True, verbose=False, hcube_ii='',DEBUG=False):
    ...
    sig=5.: Float scalar
        Value representing the number of "sigmas" above or below the "median" of the neighbouring pixel, to consider a pixel as bad. See details on parameter "m" of function reject_outlier
    ...
    obj_tmp, bpix_map_cumul: both array_like
        ...
        bpix_map_cumul: frame or cube containing the number of times each pixel was corrected for being considered a bad pixel (can be > 1 as it is an iterative process).
...
def reject_outliers(data, test_value, m=5., stddev=None, DEBUG=False):
    ...
    - scaling each element of "d" by the median value of "d" gives the absolute distances "s" of each element
    - each "s" is then compared to "m" (parameter): if s < m, we have a good neighbour, otherwise we have an outlier.
    ...
    data: array_like
        Input array with respect to which either a test_value or the central a value of data is determined to be an outlier or not
    m: float, optional
        Criterion used to test if test_value is or pixels of data are outlier(s) (similar to the number of "sigma" in std_dev statistics)
    ...
    stddev: float, optional (but strongly recommended)
        Global std dev of the non-PSF part of the considered frame. It is needed as a reference to know the typical variation of the noise, and hence avoid detecting outliers out of very close pixel values. If the 9 pixels of data happen to be very uniform in values at some location, the departue in value of only one pixel could make it appear as a bad pixel. If stddev is not provided, the stddev of data is used (not recommended).

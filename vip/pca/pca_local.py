...
from ..calib import cube_derotate,scale_cube,check_PA_vector,check_scal_vector
...
def annular_pca(array, var_list, radius_int=0, asize=2., delta_thr=1, 
    ...
    var_list : array_like, 1d
        Vector of parallactic angles (if variation = 'adi') or scaling factors 
        (if variation = 'ifs').
    ...
    delta_thr : int, optional
        Factor for increasing the parallactic angle (adi) or radial displacement
        (ifs) threshold, expressed in FWHM.
    ...
    variation: {'adi','ifs'}
        Defines the type of dataset, and hence if delta_thr will define the 
        minimum azimuthal (adi) or radial (ifs) displacement
    ...
    if min_frac >= 1: 
        msg = 'Not enough reference frames. Please reduce min_npca_lib.'
    ...
    for ann in xrange(n_annuli):
        ...
        #***********************************************************************
        # We make the matrix of the given annulus. Centering the matrix if 
        # needed removing temporal mean. 
        #***********************************************************************
        ...
        for frame in xrange(n): # for each frame 
            if threshold != 0: #threshold either in PA or scaling factor
                if ann_center > fwhm*10:        ### TBD: fwhm*10
                    indices_left = find_indices(var_list, frame, threshold, 
                                                True)
                else:
                    indices_left = find_indices(var_list, frame, threshold, 
                                                False)
                ...
                if data_ref.shape[0] <= min_npca_lib:
                    pdb.set_trace()
                    msg = 'Too few frames left in PCA ref. frames library.'
    ...
    if variation == 'adi':
        ...
        cube_var, frame,_,_,_,_ = scale_cube(array_out, var_list,
...
def subannular_pca(array, var_list, radius_int=0, asize=2., delta_thr=1, 
    ...
    reject_quads: Boolean
        If True, some quadrants will not be pca treated, depending on the 
        condition (see function quad_conditions for case per case conditions).
    ...
    if reject_quads:
        in_r_cond,out_r_cond,qua_list,min_r_qua = quad_conditions(reject_quads,
                                                                  y,x)
    ...
    for ann in xrange(n_annuli):
        ...
        for quadrant in xrange(4):
            ...
            if reject_quads:
                if ((inner_radius > in_r_cond and inner_radius < out_r_cond) or
                    (quadrant in qua_list and inner_radius > min_r_qua)):
                    continue
            ...
            for frame in xrange(n):                                             
                ...
                #***************************************************************
                V = svd_wrapper(data, svd_mode, ncomp, debug=False, 
                                verbose=False)
    ...
    if variation == 'adi':
        ...
        cube_var, frame,_,_,_,_ = scale_cube(cube_out, var_list,
                                             full_output=True,inverse=True,
                                             y_in=y_in,x_in=x_in)
...
def subannular_pca_parallel(array, var_list, radius_int=0, asize=2., 
                            delta_thr=1, ncomp=None, instrument=None, 
                            fwhm=None, center=True, nproc=None, 
                            svd_mode='arpack', full_output=False, verbose=True,
                            debug=False, variation='adi'):
    ...
    min_npca_lib = 5
    min_frac = 2.*min_npca_lib/n # This way I am pretty sure to be left with 
                                 # more than min_npca_lib frames in the PCA lib
    if min_frac >= 1: 
        msg = 'Not enough reference frames. Please reduce min_npca_lib.'
        raise ValueError(msg)
    for ann in xrange(n_annuli):
    ...
    #***************************************************************************
    # Cube is derotated/descaled according to the parallactic angle/scaling 
    # factor, and median combined
    #***************************************************************************
    if variation == 'adi':
        cube_var, frame = cube_derotate(cube_out, var_list)
    elif variation == 'ifs':
...
def get_fwhm(instrument):
    ...
    in a dictionary in vip/conf/param.py.                                    
    ### TODO: check final location
    """
    if instrument == 'naco27':                                                  
        ...
        if 'fwhm' in VLT_SINFONI:
            ...
            tel_diam = VLT_SINFONI['diam']
            fwhm = VLT_SINFONI['lambdabrg']/tel_diam*206265/VLT_SINFONI['plsc']
    elif not instrument:
...
def define_annuli(var_list, ann, n_annuli, fwhm, radius_int, annulus_width,
    ...
    Returns the parallactic angle (adi) or radial motion (ifs) threshold, the 
    inner radius and the annulus
    ...
    if threshold >= mid_range - mid_range * min_frac:
        ...
        if verbose:
            if variation == 'adi':
                 ...
                 msg = 'Scaling factor threshold {:.2f} is too big,'+ \
                       ' will be set to {:.2f}'
            print msg.format(threshold, new_pa_th)
        threshold = new_pa_th
                         
    if verbose:
        msg2 = 'Annulus {:}, thresh = {:.2f}, Inn radius = {:.2f},'+ \
               'Ann center = {:.2f}'
...
def find_indices(var_list, frame, thr, truncate, variation='adi'):
    ...
    for i in xrange(0, frame):
        if (min(np.abs([var_list[frame]-var_list[i],
                        var_list[frame]-var_list[i]+360,
                        var_list[frame]-var_list[i]-360])) < thr):
            index_prev = i
            break
        else:
            index_prev += 1
    for k in xrange(frame, n):
        if (min(np.abs([var_list[k]-var_list[frame],
                        var_list[k]-var_list[frame]+360,
                        var_list[k]-var_list[frame]-360])) > thr):
            index_foll = k
            break
        else:
            index_foll += 1
    ...
    if truncate:
        thr = min(int(n/2.),100)     ### leaving min(n/2,100) closest frames
                                     ### because for ifs pca with SINFONI
                                     ### 1970 frames/2 is too much
        q = int(thr/2.)
        if frame < thr: 
...
def get_ncomp(data, mode, debug):    ### TODO: redefine every m frames?
    ...
    while px_noise_decay > 10e-5:
        ...
        #print 'ncomp {:} {:.4f} {:.4f}'.format(ncomp, px_noise[-1], 
        #                                       px_noise_decay)
...
def quad_conditions(reject_quads,y,x,verbose=False):
    ...
    zero-padding, the svd cannot converge.

    Parameters:
    -----------
    reject_quads: str, {'HD100546_May2014','HD100546_Jan2014',...}
        String representing the dataset that is to be reduced. It is the 
        concatenation of the source name and observation date. If the name is 
        not recognized, there is no limitation in the portions of annuli that 
        are reduced with pca.
    y, x: int
        y and x dimensions of the frames of the cube to be reduced with pca.

    Returns:
    --------
    min_rad, max_rad: float
       [min_rad,max_rad] correspond to the radius range (in px) for which any 
       annulus should be rejected (in any quadrant), e.g. when dominated by 
       zero-padding.
    qua_list: set of max. 4 integers (from 0 to 3)
       Quadrants to be rejected, above a radius of min_r_qua 
       Note: quadrants are defined as such:
       quad 2 _|_ quad 3
       quad 1  |  quad 0
    min_r_qua: float
       Radius above which the quadrant list restriction starts to apply.
    """

    if reject_quads == 'HD100546_May2014':
        min_rad = min(int(y/2.),int(x/2.))   # in px; checked by eye in the cube

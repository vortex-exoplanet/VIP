...
def test_median_object(injected_cube_position):
    ...
    position = np.load(f"{PSFADI_PATH}pca_adi_detect.npy")
    exp_frame = np.load(f"{PSFADI_PATH}pca_adi.npy")
    ...
    for pca_test in pca_test_set:
        ...
        position = np.load(f"{PSFADI_PATH}{case_name}_adi_detect.npy")
        exp_frame = np.load(f"{PSFADI_PATH}{case_name}_adi.npy")

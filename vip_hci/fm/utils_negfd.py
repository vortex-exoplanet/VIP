...
from ..preproc import (cube_crop_frames, frame_pad, cube_shift, frame_shift,
                       cube_derotate, frame_rotate)
from .fakedisk import cube_inject_fakedisk


def cube_disk_free(disk_parameter, cube, angs, disk_img, psfn=None, 
                   imlib='vip-fft', interpolation='lanczos4', imlib_sh='vip-fft', 
                   interpolation_sh='lanczos4', transmission=None, weights=None, 
                   **rot_options):
    """
    ...
    disk_parameter: numpy.array or list or tuple
        The (delta_x, delta_y, theta, flux) for the model disk image. For a 4d 
        cube, disk_parameter should be either (i) a numpy array with shape 
        (4,n_ch), (ii) a 4-element list/tuple where delta_x, delta_y, theta can 
        be floats or 1d-arrays, but flux mandatorily as a 1d array with length 
        equal to cube.shape[0].
    cube: numpy.array
    ...
    if cube.ndim == 4:
        if len(disk_parameter[-1]) != cube.shape[0]:
            msg = "The length of flux scaling parameter should match cube axis 0"
        ...
        if not isinstance(disk_parameter, np.ndarray):
            ...
            if cond1 or cond2 or cond3:
                ndisk_parameter = np.zeros([4, cube.shape[0]])
                for j in range(4):
        ...
        disk_model_tmp = cube_derotate(disk_model_tmp, -angs, imlib=imlib,
                                       interpolation=interpolation, 
                                       **rot_options)
        for j in range(cube.shape[0]):
            flevel = -disk_parameter[3, j]
        ...
        delta_y = disk_parameter[1]
        theta = disk_parameter[2]
        flevel = -disk_parameter[3]
        disk_model_tmp = frame_shift(disk_model_tmp, delta_y, delta_x,
                                     imlib=imlib_sh,
                                     interpolation=interpolation_sh)
        disk_model_tmp = frame_rotate(disk_model_tmp, theta, imlib=imlib,

...
class Scattered_light_disk():
    ...
    def __init__(self,nx=200,ny=200,distance=50.,itilt=60.,omega=0.,\
        ...
        density_dico : dict
            ...
            linear flaring. This dictionary should at least contain the key "name".
            For a to-power law distribution, you can set it with 'name:'2PowerLaws'
        ...
        self.xdo=xdo # disk offset along the x-axis in the disk 
                     # frame (semi-major axis), AU
        self.ydo = ydo # disk offset along the y-axis in the disk frame 
                       # (semi-minor axis), AU 
        ...
        self.x_map_0PA, self.y_map_0PA = np.meshgrid(self.x_vector, self.y_vector)
        self.set_pa(pa)
    ...
    def set_inclination(self,itilt):
        ...
        """
        self.itilt=float(itilt) #inclination wrt the line of sight in degrees
    ...
    def set_phase_function(self,spf_dico):
        ...
        spf_dico :  dict
            Parameters describing the scattering phase function to be implemented.
            Three phase functions are implemented so far: single Heyney Greenstein,
            double Heyney Greenstein and custum phase functions through 
            interpolation. Read the constructor of each of those classes to
            know which parameters must be set in the dictionary in each case.
        """
        self.phase_function = Phase_function(spf_dico=spf_dico)
    ...
    def print_info(self):
        ...
        print('Pixel size: {0:.4f} arcsec/px or {1:.2f} au/px'.format(\
              self.pxInArcsec,self.pxInAU))
        print('Distance of the star {0:.1f} pc'.format(self.distance))        
        print('From {0:.1f} au to {1:.1f} au in X'.format(self.x_vector[0],\
              self.x_vector[self.nx-1]))
        print('From {0:.1f} au to {1:.1f} au in Y'.format(self.y_vector[0],\
              self.y_vector[self.nx-1]))        
    ...
    def check_inclination(self):
        ...
        """
        if np.abs(np.mod(self.itilt,180)-90) < \
            np.abs(np.mod(self.dust_density.dust_distribution_calc.itiltthreshold,180)-90):
            print('Warning the disk is too close to edge-on')
            print('The inclination was changed from {0:.2f} to {1:.2f}'.format(\
                  self.itilt,self.dust_density.dust_distribution_calc.itiltthreshold))
    ...
    def compute_scattered_light(self,halfNbSlices=25):
        ...
        #distance along the line of sight to reach the disk midplane (z_D=0), AU:
        lz0_map =  self.y_map * np.tan(np.deg2rad(self.itilt))
        #distance to reach +zmax, AU:
        ...
        for il in range(nbSlices):
            ...
            d2star_vector = xd_vector**2+yd_vector**2+zd_vector**2 # squared distance to the star, in AU^2
            dstar_vector = np.sqrt(d2star_vector) # distance to the star, in AU
            rstar_vector = np.sqrt(xd_vector**2+yd_vector**2) # midplane distance to the star (r coordinate), in AU
            ...
            r_vector = np.sqrt((xd_vector-self.xdo)**2+(yd_vector-self.ydo)**2) # midplane distance to the disk center (r coordinate), in AU
            theta_vector = np.arctan2(yd_vector-self.ydo,xd_vector-self.xdo) # polar angle in radians between 0 and pi
            costheta_vector = np.cos(theta_vector-np.deg2rad(self.omega))
            # Scattered light:
            rho_vector = self.dust_density.density_cylindrical(r_vector,costheta_vector,zd_vector) # volume density
...
if __name__=='__main__':
    """
    Main of the class. 
    It is an example of use of the Scattered_light_disk class. 
    """    
    ...
    #            phase function
    Scattered_light_disk1 = Scattered_light_disk(nx=201,ny=201,distance=20,\
                            itilt=70.,omega=0.,pxInArcsec=0.01225,pa=45.,\
                            density_dico={'name':'2PowerLaws','ain':5,'aout':-5,\
                               'a':40,'e':0,'ksi0':1.,'gamma':2.,'beta':1.},\
    ...
    # If you prefer set the parameters in differen steps, you can also do that with:
    # (this is identical)
    Scattered_light_disk1 = Scattered_light_disk(nx=201,ny=201,distance=20)
    ...
    Scattered_light_disk2 = Scattered_light_disk(nx=201,ny=201,distance=20)

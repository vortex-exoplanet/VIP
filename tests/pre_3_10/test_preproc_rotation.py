...
from tests.helpers import np, aarc, parametrize
from vip_hci.preproc.derotation import cube_derotate, _define_annuli, _find_indices_adi
...
@parametrize(
    "imlib,interpolation,border_mode,edge_blend",
    [
...
@parametrize(
    "imlib,interpolation,border_mode,edge_blend,interp_zeros",
    [
...
@parametrize(
    "frame, nframes, out_closest, truncate, max_frames, truth",
    [
...
def test_find_indices_adi(frame, nframes, out_closest, truncate, max_frames, truth):
    angles = np.array([130, 120, 90, 60, 30, 10, 0])

    indices = _find_indices_adi(
        angles,
        frame=frame,
        thr=42,
        nframes=nframes,
        out_closest=out_closest,
        truncate=truncate,
